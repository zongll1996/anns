...
switch type, % different algorithms
  ...
  case 'lsh',  % optimal for Hamming spaces
   ...
   % include理应包括1:d的所有维度，故include=[1:d]
   % 对每一个哈希表执行以下操作，哈希表的参数定义如下，最重要的还是I(j).d表示第j个表选取哪k个维度，也即哈希函数
   for j=1:l
     % select random dimensions
     % 随机选取k个维度
     % 其中unidrnd(length(include),1,k)表示从过滤后的维度中随机选取k个维度组成一行k列的向量
     I(j).d = include(unidrnd(length(include),1,k));
     % for each dimension select a threshold
     % hash key = [[ x(:,d)' >= t ]]
     % 从0-1中随机产生均匀分布的1行k列向量
